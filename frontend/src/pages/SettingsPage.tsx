...
export function SettingsPage({
  ...
  const [message, setMessage] = useState('Test SMS dalla dashboard Appointment Reminder');
    ...
    <div className="space-y-5">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1fr_auto] xl:items-center">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300">
              Monitora modalità ambiente, provider configurati, cron automatico e invia SMS di test senza esporre token o credenziali sensibili.
            </p>
          </div>

          <div className={isProduction ? 'rounded-[2rem] border border-emerald-400/20 bg-emerald-400/10 p-5' : 'rounded-[2rem] border border-amber-400/20 bg-amber-400/10 p-5'}>
            <div className="flex items-center gap-3">
              {isProduction ? <ShieldCheck className="h-8 w-8 text-emerald-200" /> : <AlertTriangle className="h-8 w-8 text-amber-200" />}
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Modalità</p>
                <p className={isProduction ? 'text-2xl font-black text-emerald-100' : 'text-2xl font-black text-amber-100'}>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <ProviderCard
          ...
          detail={status?.providers.twilioSms.from ? `Mittente ${status.providers.twilioSms.from}` : 'Numero mittente non configurato'}
          icon={Smartphone}
        />
        ...
        <ProviderCard
          ...
          configured={Boolean(status?.providers.telegram.configured)}
          detail={status?.providers.telegram.configured ? 'Bot token configurato' : 'Bot token mancante'}
          icon={MessageCircle}
        />
        ...
        <ProviderCard
          ...
          detail={status?.providers.whatsapp.from ? `Mittente ${status.providers.whatsapp.from}` : 'Mittente WhatsApp non configurato'}
          icon={Radio}
        />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
        <Card>
          ...
          <div className="mt-5 grid gap-3">
            <InfoRow icon={TimerReset} label="Cron reminder" value={status?.cron ?? 'Non configurato'} />
            <InfoRow icon={Clock3} label="Timezone" value={status?.timezone ?? 'Non configurata'} />
            <InfoRow icon={ServerCog} label="Stato lettura" value={isLoading ? 'Aggiornamento...' : 'Sincronizzato'} />
          </div>

          <div className="mt-5 rounded-3xl border border-white/10 bg-slate-950/45 p-4 text-sm leading-6 text-slate-400">
            I valori sono solo in lettura. Per modificarli cambia il file <span className="font-bold text-slate-200">.env</span> o <span className="font-bold text-slate-200">docker-compose.yml</span> e riavvia Docker.
          </div>
        </Card>
        ...
        <Card>
          <div className="mb-5 flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Usa questo test per verificare Twilio senza creare una prenotazione.
              </p>
            </div>

            <span className={isProduction ? 'rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-200' : 'rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1 text-xs font-bold text-amber-200'}>
          ...
          <div className="grid gap-4">
            ...
            <Button
              onClick={() => testSmsMutation.mutate({ phoneNumber, message })}
              disabled={testSmsMutation.isPending || !phoneNumber || !message}
            >
              <Send className="h-4 w-4" />
              {testSmsMutation.isPending ? 'Invio in corso...' : 'Invia SMS di test'}
            </Button>

            <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4 text-xs leading-5 text-slate-400">
              In trial Twilio il destinatario deve essere verificato nella console Twilio. Il token non viene mai mostrato nella dashboard.
            </div>
          </div>
        </Card>
      </div>
...
function ProviderCard({
  ...
  return (
    <Card className="relative overflow-hidden">
      <div className={configured ? 'absolute right-0 top-0 h-28 w-28 rounded-full bg-emerald-500/20 blur-3xl' : 'absolute right-0 top-0 h-28 w-28 rounded-full bg-rose-500/20 blur-3xl'} />
